...
export function useSystemManager(): [boolean, (enable: boolean) => void] {
  const dispatch = useDispatch<AppDispatch>()
  const systemEnable = useSelector<AppState, AppState['user']['systemEnable']>((state) => state.user.systemEnable)

  const toggleSystemEnable = useCallback((enable: boolean) => {
    if (!enable) {
      dispatch(disableSystem())
    } else {
      dispatch(enableSystem())
    }
  }, [dispatch])

  return [systemEnable, toggleSystemEnable]
}
